...
- 已实现候选：local/meta/platform 包、自动 resolver、签名与私有缓存、IPC managed、显式离线 kit；Mac/Linux 实际 npm/pnpm 安装通过。父 IPC 断开正常回收已有测试，Runtime 强杀/脱组后代的完整 containment、Windows 收口和正式发布仍未完成。
