...
**状态**：T1 本机制品/原始锁转移与零链接目录探针已通过，Mac producer→三平台 consumer CI 已接通但未远端执行；发布 metadata 与 payload 契约仍待收口，T2–T7 未开始。新增包仍不是已发布能力。
...
| P1D 统一本地安装          | `[~]` | T1 本机原始 kit/lock 重装与零链接候选目录通过       | 跨系统 CI、发布 metadata 清理与契约冻结          |
