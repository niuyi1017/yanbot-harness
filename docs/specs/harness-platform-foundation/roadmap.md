...
**状态**：T1 本机原始 kit/lock 转移、零链接目录及发布 metadata 归一化探针已通过，制品/resolver/IPC 字段已细化；Mac producer→三平台 consumer CI 已接通但未远端执行。精确解包器与安全攻击样例、平台权限/生命周期门禁仍待完成，T2–T7 未开始。新增包仍不是已发布能力。
...
| P1D 统一本地安装          | `[~]` | T1 本机 kit/lock、零链接与 metadata 归一化通过，字段契约已细化 | 精确解包器/攻击样例、跨系统 CI 与平台安全门禁    |
