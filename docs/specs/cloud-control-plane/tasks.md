...
- [x] 新增 NestJS 11 `apps/cloud-server`、严格配置、Remote health 与安全 middleware。
- [x] 定义 tenant-first repository ports、Mongo schemas、复合唯一索引、TTL 索引与 transaction helper。
- [x] 增加确定性 memory store 仅供 service 单测；production bootstrap 禁止使用。
- [x] 增加 bootstrap provision、index sync 与 workspace cleanup 运维命令。

验收：build/typecheck 通过；缺配置/production 无 TLS 声明 fail closed；集合前缀和索引测试通过。

## T4. Identity、认证与审计

- [x] 实现 organization/user/membership/device provision 与禁用状态。
- [x] 实现 device exchange、短期 access、refresh 单次轮换、reuse family revoke 与 access guard。
- [x] 实现固定字段 audit service 和公共错误映射。
- [x] 覆盖无效/过期/撤销/replay/跨组织与 secret marker 脱敏测试。

验收：raw secret 不进入 store 序列化/audit/error；每个业务请求重新验证身份状态。

## T5. Workspace Preparation

- [x] 实现 tenant-scoped snapshot upload staging、验证、原子发布与失败清理。
- [x] 实现 allowlisted HTTPS + immutable commit Git registration；API 不联网。
- [x] 实现 workspace TTL、Run 创建时 expiry 验证与 owned cleanup。
- [x] 覆盖 traversal、symlink、collision、digest、oversize、SSRF URL、userinfo/query 与跨租户负例。

验收：只有受控 workspaceRef 进入 Run；数据库或文件失败不留下可用半成品。

## T6. Session、Run、Outbox、Event 与 SSE

- [x] 实现 Reference catalog、Session CRUD 和 tenant 隔离。
- [x] 实现 workspace-bound Run、Session 写锁、fingerprint 幂等和 transaction outbox。
- [x] 实现持久化 Event append、cursor replay、SSE polling、唯一终态与取消竞态。
- [x] 实现 Interaction response 持久化/幂等。

验收：公共 Conformance 中无需 Worker 的资源语义通过；Run 保持 queued，API 进程不导入/执行 Adapter。

## T7. Execution Grant 与内部接口

- [x] 实现 run/workspace/attempt/action scoped grant 签发、单次 claim、过期和撤销。
- [x] 内部 endpoint 默认关闭，开启后只接受 grant，不接受用户 access token。
- [x] 覆盖错误 Run/attempt/action、重复 claim、终态后 append 与 raw grant 脱敏。

验收：grant 不能列举资源或跨 Run 使用；outbox、event、audit 中不存在 raw grant。

## T8. SDK/CLI Remote Workspace 接入

- [x] SDK 增加 snapshot/Git preparation 方法，不接收本机路径作为 Remote source。
- [x] CLI 在显式 snapshot/Git 参数存在时启用同名 Remote `run`；默认 cwd 仍不得发送。
- [x] 更新 profile/CLI 文档和请求捕获测试。
