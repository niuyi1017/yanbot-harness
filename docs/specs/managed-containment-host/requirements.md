...
- CH8：在自有临时消费者中验证冻结 preview.2 → 测试签名 VM preview.3 → preview.2 的 Reference 业务状态往返；使用两个版本各自安装的 SDK/Runtime，保留 Session/Run/Event 和工作区哨兵，不覆盖旧发布物。该证据不代替两个正式版本、真实厂商或通用数据迁移认证。
