...
- [x] 实现严格 target/runner 绑定与 Git source metadata。
- [x] 使用真实 loopback HTTP/SSE Local driver 运行四项共享 Conformance。
- [x] 使用 injected-fetch Remote Fixture driver运行同四项共享 Conformance。
- [x] 无论成功或失败均原子写入脱敏 JSON；失败时返回非零退出码。

验收：本机生成的 darwin 报告字段、身份、四场景和清理行为均符合 Spec。

## T3. 聚合器与负例

- [x] 实现严格 platform report validator 与双 target 聚合器。
- [x] 拒绝缺失/重复 target、commit 不同、runner 不匹配、场景失败和 Fixture 冒充 service。
- [x] 输出 `status: incomplete` 和独立 Remote service 缺口。
- [x] 增加 Node 单测并纳入 `test:probes`。

验收：只有同 commit 的 macOS/Windows 已通过报告可被聚合，且聚合结果不能声称 Remote service 通过。

## T4. CI 矩阵

- [x] 新增 macos-15/darwin-arm64 与 windows-2022/win32-x64 独立 matrix job。
- [x] 每个 runner frozen install、build、生成并上传独立证据。
- [x] aggregate job 下载明确 artifact，生成并上传 summary。
- [x] workflow 不使用 Secret，不访问真实模型或公网 Runtime。
